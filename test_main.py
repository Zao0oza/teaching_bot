...
@pytest.mark.asyncio
async def test_quick_if_pupil_exist():
...
@pytest.mark.skip
@pytest.mark.asyncio
async def test_complete_with_new_pupil():
    '''Прогоняем бота по всему курсу'''
    controller = BotController(
        peer="@rusteach_bot",  # We are going to run tests on https://t.me/BotListBot
        client=create_client(),
    ...
    await controller.send_command("test_pupil", add_bot_name=False)
    async with controller.collect(count=1) as response:  # type: Response
        await controller.send_command("да", add_bot_name=False)
    assert response.num_messages == 1

    async with controller.collect(count=1) as response:  # type: Response
        await controller.send_command("wrong_answer", add_bot_name=False)
    assert response.num_messages == 1
