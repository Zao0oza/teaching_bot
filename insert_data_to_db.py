...
def insert_for_chinese_db():
    start_messages = ['Давай сегодня поработаем как следует и разберёмся с одной из тех грамматических тем, которые не '
    ...
    for file in files:
        if os.path.isdir(os.path.join(dirname, file)):
            continue
        image_to_db(os.path.join(dirname, file),1)
    for file in files2:
        if os.path.isdir(os.path.join(dirname2, file)):
            continue
        image_to_db(os.path.join(dirname2, file),1, True)
